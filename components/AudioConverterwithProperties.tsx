...
export default function AudioConverterWithProperties({
	format,
	primaryFormat,
}: { format: string; primaryFormat: string }) {
  ...
  const ButtonText = () => {
	...
	const FFMPEGProcessor = async () => {
		...
		const attributes = [...codec, ...channel, ...vol, ...sr, ...audioBR];
				...
				<div className={`${converting && inputFile ? "blur disabled" : ""}`}>
					...
					<div className="mt-4 grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 xl:gap-14 lg:gap-12 md:gap-8 gap-6 items-center">
						<div className="flex flex-col">
							...
							>
								Audio Codec{" "}
								<InfoTooltip information="An Audio Codec is a software or hardware tool that converts (encodes or decodes) audio data from one format to another." />
							</label>
							<Select
                // biome-ignore lint/suspicious/noExplicitAny: <explanation>
								onValueChange={(value: string | any) =>
									handleAudioCodecChange(value)
								}
							>
